...
// 🔴 حالات حقيقية من محادثات زبائن فشلت سابقاً — تُقفل بالاختبار
console.log("\n── قرى وكلمات ملتصقة (حالات زبائن حقيقية) ──");
for (const [txt, area, gov] of [
  ["كفراسد", "كفر اسد", "اربد"],            // ملتصقة بلا مسافة
  ["اربدكفراسد", "كفر اسد", "اربد"],        // محافظة+قرية ملتصقات
  ["اربد كفر أسد", "كفر اسد", "اربد"],
  ["عمانالبيادر", "البيادر", "عمان"],
  ["بيتراس", "بيت راس", "اربد"],
  ["كفر سوم", "كفر سوم", "اربد"],
  ["سما الروسان", "سما الروسان", "اربد"],
  ["ام قيس", "ام قيس", "اربد"]
]) {
  const r = parseAddress(txt, { fuzzy: true });
  ok(r.components.area === area && r.components.governorate === gov && r.deliverable,
     `"${txt}" ⇒ ${r.components.area || "🔴 فشل"} / ${r.components.governorate || "—"}`);
}
// الرسالة الحقيقية كاملة من محادثة أبو أسامة (كانت تفشل ويظل البوت يسأل)
{
  const full = "بدي جالون 20 لتر.اربدكفراسد ابو أسامه الرياشي ت 0772227832 بدي ريحه كويسه";
  const r = parseAddress(full, { fuzzy: true });
  ok(r.deliverable && /كفر اسد/.test(r.formatted),
     `رسالة زبون كاملة ⇒ "${r.formatted}" (يوصل: ${r.deliverable})`);
}
